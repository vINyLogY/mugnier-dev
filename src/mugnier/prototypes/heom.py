    ...
    if dry_run:
        ...
        for _n, _t in zip(range(steps), propagator):
            if (_n + 1) % callback_steps == 0:
                ...
                print({n:propagator.state.shape(n) for  n in propagator._node_visitor}, flush=True)
                coh = abs(rdo[0, 1])
    ...
    for _ps_method, _dt, _end, _callback in zip(['vmf', 'ps1'], dt, end,
        ...
        for _n, _t in zip(range(_steps), _propagator):
            if (_n + 1) % callback_steps == 0:
                ...
                print({n:_propagator.state.shape(n) for  n in _propagator._node_visitor}, flush=True)
                coh = abs(rdo[0, 1])
